...
<img src="https://img.shields.io/badge/jsDocs.io-reference-blue" alt="jsDocs.io"/>
<br/>
...
<Warning>You must supply your `privateKey` to the `initialize` function in order to use secrets.</Warning>
...
const showcaseMgmtFeatures = async () => {
  ...
  try {
    await enrolla.createCustomer({ // create a new customer in Enrolla with the supplied organizationId
      name: 'Customer Name',
      organizationId,
      pack
    });
