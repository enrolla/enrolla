...
- An *entitlement*, like which product features are enabled for a customer.
- A *configuration*, which can be any customer specific setting (e.g. a color scheme, cloud provider. etc.).

**Each `feature` includes:**
|           |                                                                                    |
|---------------|-----------------------------------------------------------------------------------------------|
| __Key__           | The name of the feature. (it must be a unique string, with no whitespace)                                      |
| __Type__          | One of the following: `string`, `integer` ,  `float` , `boolean` ,  `JSON`. Once set, this value cannot be changed. |  
| __Default Value__ | The value that this feature will be set to for all customers unless otherwise overridden.                       | 
| __Description__   | Free text to help you identify the feature.                                                                      |
