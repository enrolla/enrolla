...
import { Space, TextInput, Text, Group, Badge, Button, Modal, Title, Container } from '@pankod/refine-mantine';
...
const arrToMap = (arr: SecretInput[], status: Status) => arr.reduce((acc, s) => {
    acc[s.key] = {
        ...s,
        status
    };
...
const SecretBadge = ({ secret }: { secret: DisplaySecret }) => {
    ...
    return (
        <Badge style={{ width: '10% '}}color={badgeColor}>{badgeText}</Badge>
    )
}
...
const SecretDisplayAndEditComponent = ({
    ...
    return (
        ...
        <Group>
            ...
            {secret.status == STATUS.EDITED && <Button color="orange" size="xs" compact onClick={() => onUndo(secret.key)}>
                Undo
            </Button>}
        </Group>
        </>
...
export const SecretsEditComponent = ({
  ...
  for (let sk of secretKeys) { // Add any missing keys, which are not in the existing secrets
    const k = sk.key;
    if (!displaySecretMap[k]) {
        displaySecretMap[k] = { key: k, value: '', status: STATUS.NOT_DEFINED };
    }
  }
  for (let es of editedSecrets) { // Override with edited secrets
    displaySecretMap[es.key] = { ...es, status: STATUS.EDITED };
  }

  const displayArr = Object.values(displaySecretMap).sort((a, b) => statusCompare(a.status, b.status));

  const onUpdate = (key: string, value: string) => {
    if (!value) {
      return;
    }
    onSecretsChange(Object.values({ ...editedSecretMap, [key]: { key, value } }));
  };
